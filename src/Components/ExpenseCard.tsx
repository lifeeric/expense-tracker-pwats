...
const ExpenseCard = () => {
  ...
  return (
    ...
    <div className="Balance-Card shadow-sm rounded mt-4 p-3">
      ...
      <h4>Your Balance</h4>
      <h3>
        <strong>${grossIncome() + totalExpense()}</strong>
      </h3>
      <Card>
        <Card.Body>
          <Row>
            <Col>
              ...
              <h4 className="text-center text-success">
                <strong>${grossIncome()}</strong>
              </h4>
            </Col>
            <Col>
              <h4 className="text-center">Expense</h4>
              <h4 className="text-center text-danger">
                <strong>-${Math.abs(totalExpense())}</strong>
              </h4>
            </Col>
          </Row>
        </Card.Body>
          ...
          <Alert key={index} variant={`${chec ? "success" : "danger"}`}>
            <button
              ...
              className={`delete-btn ${
                chec ? "delete-success" : "delete-danger"
              }`}
            >
              X
            </button>
            <p className="History-record">
              <span>{transObj.desc}</span>
              <span>
                {sign}${Math.abs(transObj.amount)}
              </span>
            </p>
          </Alert>
        );
